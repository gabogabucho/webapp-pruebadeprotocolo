...
- **Descripción**: La app tiene un *light theme* elegante pero carece de modo oscuro. Hay que agregar un botón de *luna/sol* en el *header* y manejar un estado `data-theme="dark"` en el `<html>` o en el `<body>`.
- **Qué archivos modificar**: `index.html` (header), `index.css` (variables CSS y clases auxiliares), `app.js` (persistencia del user preference en `localStorage`).
- **Dificultad**: Media.
